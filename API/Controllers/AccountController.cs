...
namespace API.Controllers
{
    public class AccountController : BaseApiController
    {
        ...
        public async Task<ActionResult<UserDto>> Login(LoginDto loginDto)
        {
            ...
            if (user == null) //for checking the emailconfirmation
            {
                return Unauthorized(new ApiResponse(401, "Please register yourself"));
            }
            if (!await _userManger.IsEmailConfirmedAsync(user))
            {
                return BadRequest(new ApiResponse(400, "Email address not confirmed yet"));
            }
